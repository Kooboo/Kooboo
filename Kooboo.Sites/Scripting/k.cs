    ...
    public class Template
    {
        ...
        public Dictionary<string, object> Upload(string domain,byte[] postdata)
        {
            ...
            if (formResult.Files.Count() > 0)
            {
                ...
                var images = formResult.Files.FindAll(f => Kooboo.Lib.Helper.UrlHelper.IsImage(f.FileName));
                if (images.Count == 0)
                {
                    //screenshot
                }
                ...
                {
                    string relativeurl = Guid.NewGuid().ToString().Replace("-", "") + Kooboo.Lib.Helper.UrlHelper.FileExtension(image.FileName);
        ...
        public byte[] GetImage(string imagename,int width=0,int height=0)
        {
