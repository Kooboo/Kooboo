...
namespace Kooboo.Sites.Scripting.Global.SMS
{
    public class Ali
    {
        ...
        [Description("k.sms.aliSMS.send(\"your_ali_template_code\", \"+8615312345678\", \"code\", \"12345\");")]
        ...
        [Description("k.sms.aliSMS.send(\"your_ali_template_code\", \"+8115312345678\", \"code\", \"12345\");")]
        public bool SendInternational(string templateCode, string ToPhoneNumber, string bindingkey, string bindingvalue)
        {
            var setting = this.Context.WebSite.SiteDb().CoreSetting.GetSetting<AliSMSSetting>();

            if (setting == null || string.IsNullOrWhiteSpace(setting.accessId) || string.IsNullOrWhiteSpace(setting.accessSecret) || string.IsNullOrWhiteSpace(setting.internationalSignName))
            {
                throw new Exception("Ali SMS service not configured");
            }
            ...
            {
                request.PhoneNumbers = ToPhoneNumber;
                request.SignName = setting.internationalSignName;
                request.TemplateCode = templateCode;
