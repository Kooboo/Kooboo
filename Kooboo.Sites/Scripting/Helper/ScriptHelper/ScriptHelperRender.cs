            ...
            if (setting.Props.Count > 0)
            {
                ...
                foreach (var prop in setting.Props)
                {
                    ...
                    if (prop.Type != null && ScriptHelperReader.Settings.ContainsKey(prop.Type.ToLower()))
                    {
                        ...
                        propStr =
                            $"<tr><td>{DocumentHelper.LowerCaseFirstChar(prop.Name)}</td><td>{type}</td><td>{prop.Description}</td></tr>";
                    }
                    else
                    {
                        propStr =
                            $"<tr><td>{DocumentHelper.LowerCaseFirstChar(prop.Name)}</td><td>{prop.Type}</td><td>{prop.Description}</td></tr>";
                    }
