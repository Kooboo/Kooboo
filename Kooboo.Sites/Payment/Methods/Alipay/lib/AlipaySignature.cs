...
namespace Kooboo.Sites.Payment.Methods.Alipay.lib
{
    public class AlipaySignature
    {
        ...
        public static string RSAEncrypt(string content, string publicKeyPem, string charset)
        {
            ...
            {
                throw new AliPayException("EncryptContent = " + content + ",charset = " + charset + ".Error message:" + ex.Message);
            }
        }
        ...
        public static string RSADecrypt(string content, string privateKeyPem, string charset, string signType)
        {
            try
            {
                var rsaCsp = LoadCertificateString(privateKeyPem, signType);
                if (string.IsNullOrEmpty(charset))
                    charset = DEFAULT_CHARSET;
            ...
            {
                throw new AliPayException("DecryptContent = " + content + ",charset = " + charset + ".Error message:" + ex.Message);
            }
        }
