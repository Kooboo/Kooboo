...
namespace Kooboo.IndexedDB.WORM.Query
{
    ...
    public class MetaCollection<TValue> : IEnumerable<TValue>
    {
        ...
        public MetaCollection(WormDb<TValue> db, LeafResult startLeaf, LeafResult endLeaf, bool ascending)
        {
            if (!TypeHelper.HasInterface(typeof(TValue), typeof(MetaObject.IMetaObject)))
            {
                throw new System.Exception("Only IMetaObject are supported");
            }
        ...
        public class Enumerator : IEnumerator<TValue>
        {
            ...
            public bool MoveNext()
            {
                if (this.readIndex <= this.itemCount - 1)
                {
                    ...
                    this.currentValue = this.Db.LoadMeta(item);
                    this.readIndex += 1;
                    return true;
                }
