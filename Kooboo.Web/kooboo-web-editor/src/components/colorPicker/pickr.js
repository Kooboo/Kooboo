...
export class Pickr {
  ...
  constructor(opt) {
    ...
    requestAnimationFrame(function cb() {
      if (!app.offsetWidth && app.parentElement !== parent.document.body) {
  ...
  _preBuild() {
    ...
    parent.document.body.appendChild(this._root.root);
  }
  ...
  _finalBuild() {
    ...
    // Remove from body
    parent.document.body.removeChild(root.root);
    ...
    if (opt.inline) {
      ...
      parent.document.body.appendChild(root.app);
    }
  ...
  _bindEvents() {
    ...
    const eventBindings = [
      ...
      _.on(
        [_root.palette.palette, _root.palette.picker, _root.hue.slider, _root.hue.picker, _root.opacity.slider, _root.opacity.picker],
    ...
    if (!options.showAlways) {
      ...
      eventBindings.push(
        ...
        _.on(parent.document, "keyup", e => this.isOpen() && (e.key === ck || e.code === ck) && this.hide()),
  ...
  addSwatch(color) {
    ...
    if (values) {
      ...
      const element = _.createElementFromString(`<button type="button" style="color: ${hsvaColorObject.toRGBA().toString(0)}"/>`);
