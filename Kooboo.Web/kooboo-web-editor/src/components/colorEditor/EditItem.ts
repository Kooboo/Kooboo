...
export default class EditItem {
  constructor(
    ...
    protected onGlobalSave: (color: string) => void = c => {},
    protected onLocalSave: (color: string) => void = c => {}
  ) {
    this.color = oldColor;
    this.isGlobal = false;
  }
  ...
  createPicker() {
    let div = createColorPicker(this.text, this.oldColor, c => {
      this.color = c;
      this.isGlobal == true ? this.onGlobalSave(c) : this.onLocalSave(c);
    });
