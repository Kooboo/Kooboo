...
import { getMatchedColorGroups, CssColorGroup, CssColor } from "@/dom/style";
...
function createItem(colorGroup: CssColorGroup, el: HTMLElement, comment: KoobooComment, koobooId: string) {
  ...
  let picker = createColorPicker(
    colorGroup.prop + colorGroup.pseudo, 
    firstCssColor.prop.getColor(firstCssColor.value), 
    s => {
    ...
    if (isGlobalUpdate) {
      ...
      classCssLog = StyleLog.createCssUpdate(value, classCssColor.prop.prop, classCssColor.rawSelector, classCssColor.koobooId, classCssColor.url!, !!classCssColor.newImportant);
    } else {
