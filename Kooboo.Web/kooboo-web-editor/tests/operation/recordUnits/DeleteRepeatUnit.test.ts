...
describe("DeleteRepeatUnit", () => {
  test("undo", () => {
    ...
    expect(nodeParent.children[0].outerHTML).toEqual("<div><p>abc</p><p>cdf</p></div>");
    expect(unit.newValue).toEqual("<!--abcde-->");
  });
  ...
  test("redo", () => {
    ...
    expect(nodeParent.innerHTML.trim()).toEqual("<!--abc-->");
    expect(unit.oldValue).toEqual(html);
  });
});
