...
describe("utils", () => {
    ...
    test("isViewComment", ()=>{
        ...
        expect(isViewComment(comment)).toEqual(true);
        expect(isViewComment(suportComment)).toEqual(false);
    })
    ...
    test("isEditComment", ()=>{
        ...
        expect(isEditComment(comment)).toEqual(true);
        expect(isEditComment(suportComment)).toEqual(false);
    })

    test("isRepeatComment", ()=>{
        let comment = new KoobooComment("#kooboo--objecttype='contentrepeater'--nameorid='test'--boundary='101'");
        let suportComment = new KoobooComment(
            "#kooboo--objecttype='Url'--nameorid='d8eee011-69d3-0a6c-632f-822df385ab3c'--folderid='ffa232c4-ca49-9c07-8b43-fd30d5ec5e8b'--bindingvalue='List_Item'--boundary='111'"
        );

        expect(isRepeatComment(comment)).toEqual(true);
        expect(isRepeatComment(suportComment)).toEqual(false);
    })
});
