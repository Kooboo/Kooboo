$(function() {
  var siteLogViewModel = function() {
    ...
    this.onBlameChanges = function() {
      function blame() {
        Kooboo.SiteLog.Blame(JSON.stringify(self.getSelectedIds())).then(
          function(res) {
            if (res.success) {
              location.reload();
            }
    ...
    this.isCheckoutSubmitValid = function() {
      return (
        self.checkoutSiteName.isValid() && self.checkoutSubDomain.isValid()
      );
    };
  ...
  var handleData = function(data) {
    ...
    data.list.forEach(function(log) {
      ...
      var model = {
        ...
        type: {
          text: log.storeName
            ? Kooboo.text.component.table[log.storeName.toLowerCase()] ||
              log.storeName
            : "",
        ...
        action: {
          text:
            Kooboo.text.action[log.actionType.toLowerCase()] || log.actionType,
          class:
            "label-sm " +
