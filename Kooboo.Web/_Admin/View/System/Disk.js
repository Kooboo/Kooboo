    ...
    var diskModel = function() {
        ...
        this.clearLogs = function() {
            if (confirm(Kooboo.text.confirm.disk.clearLogs)) {
                $.when(Kooboo.Disk.CleanRepository(),
                    Kooboo.Disk.CleanLog()).then(function(r1, r2) {
        ...
        Kooboo.EventBus.subscribe("kb/table/rendered", _.throttle(function() {
            chart.resize();
        }, 200));
    }
    ...
    function init() {
        Kooboo.Disk.getList().then(function(res) {
            if (res.success) {
                    ...
                    chartOption = {
                        tooltip: {
                            ...
                            formatter: function(data) {
                                return data.data.name + " - " + Kooboo.bytesToSize(data.data.value) + " (" + data.percent + "%)"
                            }
                        },
                        legend: {
                            orient: 'vertical',
                ...
                sizeArray = _.sortBy(sizeArray, [function(o) {
                    return o.length;
                }])
