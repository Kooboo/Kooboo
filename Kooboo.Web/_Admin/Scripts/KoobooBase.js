(function(wind) {
    ...
    BaseModel.prototype = {
        ...
        executeGet: function(method, data, hideLoading, useSync) {
            ...
            return DataCache.getData(this.name, method, data, useSync).fail(function(fail) {
        ...
        executePost: function(method, data, hideLoading, extendParams, useSync) {
            ...
            return DataCache.postData(this.name, method, data, extendParams, useSync).fail(function(fail) {
                handleRequestError(fail);
            }).always(function() {
                !hideLoading && loading.stop();
            }).done(function(res) {
        ...
        executeUpload: function(method, data, progressor) {
            ...
            return DataCache.uploadData(this.name, method, data, progressor).fail(function(fail) {
                handleRequestError(fail)
            }).always(function() {
    ...
    Kooboo.getResourceTagId = function(type) {
        return type + '-' + (Math.round(Math.random() * Math.pow(2, 16))) + '-' + (+new Date());
    }
        ...
        input: {
            ...
            inputNumberOnly: function(m, e) {
                if (e.keyCode >= 48 && e.keyCode <= 57 /*number*/ ) {
                    ...
                    return true;
                } else if (e.keyCode == 189 /* - */ && e.keyCode == 190 /*.*/ || e.keyCode == 69 /*e*/ || e.keyCode == 8 /*BACKSPACE*/ ) {
                    return true;
                } else if (e.keyCode == 9 /* Tab */ ) {
                    return true
                } else {
    ...
    Kooboo.isFileNameExist = function(uploadFiles, localFiles) {
        ...
        for (var i = 0, len = uploadFiles.length; i < len; i++) {
            for (var j = 0, _len = localFiles.length; j < _len; j++) {
                var localName = (typeof localFiles[j].name == "function") ? localFiles[j].name() : localFiles[j].name;
    ...
    Kooboo.loadJS = function(paths, fromLayout) {
        ...
        function loadJS(path, text, fromLayout) {
            var script = $("<script data-src=\"" + path + "\"" + (fromLayout ? "layout-script" : "spa-script") + ">" + text + "</script>");
            $("head").append(script);
        }
    }
