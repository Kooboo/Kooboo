(function() {
  ...
  ko.bindingHandlers.autoFocus = {
    init: function(
      ...
      ko.bindingHandlers.autoFocus.update(
        element,
        valueAccessor,
        allBindingsAccessor
      );
  ...
  ko.bindingHandlers.collapsein = {
    ...
    update: function(element, valueAccessor) {
      $(element).collapse(
        ko.utils.unwrapObservable(valueAccessor()) ? "show" : "hide"
      );
    }
  };
  ko.validation = {
    rules: {
      ...
      dataType: function(value, params) {
        var value = value.toString(),
          ...
          message =
            params.message ||
            Kooboo.text.validation.dataTypeInvalid +
              Kooboo.text.validation.dataType[type];
        var valid = false;
        switch (type) {
          case "Integer":
            if (!!value.match(/^\d+$/)) {
        ...
        if (!isNaN(val)) {
          if (params.value > val) {
            return {
              ...
              message:
                params.message ||
                Kooboo.text.validation.greaterThan + (params.value - 1)
            };
          } else {
            return { valid: true };
          }
          ...
          return {
            valid: false,
            message:
              Kooboo.text.error.dataType +
              ", " +
              Kooboo.text.correct.dataType.number
          };
        }
      },
        ...
        if (!isNaN(val)) {
          if (params.value < val) {
            return {
              valid: false,
              message:
                params.message || Kooboo.text.validation.lessThan + params.value
            };
          } else {
            return { valid: true };
          }
      ...
      kbDomain: function(value, params) {
        if (value) {
          return ko.validation.rules.regex(value, {
            ...
            message:
              params ||
              Kooboo.text.validation.startWithNonPunctuation +
                ", " +
                Kooboo.text.validation.onlyFollowingPunctuationAllowed +
                ". - _"
          });
        } else {
          return { valid: true };
  ...
  ko.bindingHandlers.tooltip = {
    init: function(
      ...
      ko.bindingHandlers.tooltip.update(
        element,
        valueAccessor,
        allBindingsAccessor
      );
  ...
  ko.bindingHandlers.error = {
    init: function(
      ...
      ko.bindingHandlers.error.update(
        element,
        valueAccessor,
        allBindingsAccessor,
        bindingContext
    ...
    update: function(
      ...
      function isCoverByHeader(el) {
        ...
        return (
          elemRect.top + elemRect.height < headerRect.top + headerRect.height
        );
      }
      ...
      var errorContainer =
        allBindingsAccessor().errorContainer ||
        element.getAttribute("kb-error-container");
      ...
      if (error && isShow && $el.is(":visible")) {
        if (!$el.data().hasOwnProperty("bs.tooltip")) {
          if ($el[0].getBoundingClientRect().top < 0) {
            ...
            scrollElement.scrollTop = isCoverByHeader($el[0])
              ? 68
              : $el[0].getBoundingClientRect().top + 20;
          } else if (
          ...
          $el.tooltip({
            ...
            trigger: "manual",
            template:
              '<div class="tooltip error" role="tooltip" style="' +
              (errorContainer ? "position: absolute" : "z-index:199999") +
  ...
  ko.bindingHandlers.modal = {
    init: function(
      ...
      ko.bindingHandlers.modal.update(
        element,
        valueAccessor,
        allBindingsAccessor
      );
