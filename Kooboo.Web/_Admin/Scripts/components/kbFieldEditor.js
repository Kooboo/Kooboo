(function() {
    ...
    var template = Kooboo.getTemplate("/_Admin/Scripts/components/kbFieldEditor.html");
    ...
    ko.components.register("kb-field-editor", {
        viewModel: function(params) {
            ...
            this.showMultilingualOption = ko.observable(params.options.showMultilingualOption);
            this.multilinguable = ko.observable(false);

            this.showMediaFilePanel = ko.observable(false);

            this.showDBColumnPanel = ko.observable(!!params.options.showDBColumnPanel);

            this.onShow = params.onShow;
            this.onShow.subscribe(function(show) {
                if (show) {
                    var field = new Field(_.assignIn(params.data() || { editable: true }, params.options));
                    ...
                    field.on('controlType/change', function(type) {
                        ...
                        self.multilinguable(t ? (t.dataType.toLowerCase() == "string") : false);

                        if (['boolean', 'switch', 'selection', 'fixedspec', 'dynamicspec'].indexOf(type.toLowerCase()) > -1) {
                            self.showValidationTab(false);
                        } else {
                            self.showValidationTab(true);
                        }

                        if (['selection', 'checkbox', 'radiobox', 'fixedspec'].indexOf(type.toLowerCase()) > -1) {
                        ...
                        if (type.toLowerCase() == 'mediafile') {
                            self.showMediaFilePanel(true);
                        } else {
                            self.showMediaFilePanel(false);
                        }
                    })
                    ...
                    if (self.field().isNewField()) {
                        ...
                        if (!self.field().isSystemField()) {
                            if (self.field().controlType().toLowerCase() == 'tinymce') {
                                self.field().controlType('RichEditor');
                            }
                            var type = self._controlTypes().find(function(t) {
                                    return t.value.toLowerCase() == self.field().controlType().toLowerCase();
                                }),
                                types = [];

                            var dataType = type.dataType;
                            self._controlTypes().forEach(function(t) {
                                if (t.dataType == dataType) { types.push(t); }
                            })
            ...
            this.showAddOptionPanel.subscribe(function(show) {
                if (!show) {
                    ...
                    self.field().options().forEach(function(opt) {
                        opt.showError(false);
                    })
            ...
            this.onModalSave = function() {
                if (this.showAddOptionPanel()) {
                    this.field()._optionRequired(this.field().options().length ? "OPTIONS" : "");
                }
            ...
            this.demappingField = function(field) {
                ...
                var result = {
                    ...
                    multipleValue: (field.controlType().toLowerCase() == 'checkbox' ? true : field.isMultipleValue()),
    ...
    function Field(data) {
        ...
        var _options = JSON.parse(data.options || data.selectionOptions || "[]").map(function(o) {
            return new Option(o);
        });
        ...
        this.isMultipleValue = ko.observable(data.isMultipleValue || data.multipleValue || false);
        this.isSystemField = ko.observable(data.isSystemField || false);
        var multilingual = (data.hasOwnProperty('multilingual') || data.hasOwnProperty('multipleLanguage')) ? (data.multilingual || data.multipleLanguage) : true;
        ...
        if (data.modifiedField) {
            ...
            this[data.modifiedField] = ko.observable(data[data.modifiedField] || false);
        ...
        this.isBasicValid = function() {
            ...
            if (['selection', 'checkbox', 'radiobox', 'fixedspec'].indexOf(this.controlType().toLowerCase()) > -1) {
        ...
        this.isValidationValid = function() {
            if (['boolean', 'switch', 'selection'].indexOf(this.controlType().toLowerCase()) > -1) {
                return true;
            } else {
                var flag = true;
                if (this._validations().length) {
                    this._validations().forEach(function(v) {
                        if (!v.isValid()) { flag = false; }
                    })
                }
                return flag;
            }
