(function() {
    var template = Kooboo.getTemplate("/_Admin/Scripts/textContent/folderEditor.html");
        ...
        viewModel: function(params) {
            ...
            self.hasContentType = ko.pureComputed(function() {
                var cid = self.contentTypeId(),
                    id = self.id();
                return cid && cid !== Kooboo.Guid.Empty && id && id !== Kooboo.Guid.Empty;
            });
            ...
            this.isValid = function() {
                if (self.isNew()) {
                    return self.name.isValid() && self.contentTypeId.isValid() && !self.isCatogoryAliasError() && !self.isEmbeddedAliasError();
                } else {
                    return self.contentTypeId.isValid() && !self.isCatogoryAliasError() && !self.isEmbeddedAliasError();
                }
            }
            ...
            this.showRelationFolderError = function() {
                _.forEach(_.concat(self.categoryFolders(), self.embeddedFolders()), function(folder) {
            ...
            self.submit = function(form) {
                ...
                if (self.isNew()) {
                    if (!self.name.isValid() || !self.contentTypeId.isValid()) {
                        if (!$("a[href=#tab_basic]").parent().hasClass("active")) {
                ...
                if (self.isCatogoryAliasError() || self.isEmbeddedAliasError()) {
                    if (!$("a[href=#tab_relation]").parent().hasClass("active")) {
            ...
            self.availableFolders = function(id) {
                var removeList = _.map(_.concat(self.categoryFolders(), self.embeddedFolders()), "folderId");
                var availableFolders = _.remove(_.cloneDeep(self.allFolders()), function(folder) {
            ...
            Kooboo.EventBus.subscribe("ko/textContent/folderSetting", function(selectedFolderId) {
                if (!!selectedFolderId) {
                    self.init(selectedFolderId);
                }
            });

            Kooboo.EventBus.subscribe("ko/textContent/newFolder", function(selectedFolder) {
                self.init(null);
            });
            ...
            function CategoryFolder(opt) {
                ...
                this.alias = ko.validateField(opt && opt.alias, {
                    required: Kooboo.text.validation.required,
                    localUnique: {
                        compare: function() { return self.getAliasNames(); },
                        message: Kooboo.text.validation.taken
                    }
                })
                ...
                this.alias.subscribe(function() {
                    _.forEach(_.concat(self.categoryFolders(), self.embeddedFolders()), function(folder) {
                        (folder.folderId !== _this.folderId) && folder.showError() && !folder.alias.isValid() && folder.alias.valueHasMutated();
                    })
                })
                this.showError = ko.observable(false);
            }
            ...
            function getTypeList() {
                Kooboo.ContentType.getList().then(function(res) {
                    contentTypes = _.concat([{ id: "", name: Kooboo.text.component.folderEditor.chooseItemBelow }], res.model);
