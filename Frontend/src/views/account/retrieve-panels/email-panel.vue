...
import { ResetByEmail, EmailCode } from "@/api/password/index";
...
const rules = {
  email: [requiredRule(t("common.inputEmailTips")), emailRule],
...
const emailValidator = new Schema({ email: rules.email });
const form = ref();
const router = useRouter();

const onReset = async () => {
  await form.value.validate();
  const result = await ResetByEmail(model.email, model.code, model.password);
  if (result.error) {
...
const validateEmail = async () => {
  emailValidator.validate({ email: model.email }, (error) => {
