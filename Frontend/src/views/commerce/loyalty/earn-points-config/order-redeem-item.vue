    ...
    <div class="flex items-start">
      <div class="flex flex-col gap-8 flex-1">
        <ElRadioGroup v-model="item.isPercent">
          <ElRadio :label="false">{{ t("commerce.maxRedeemPoints") }}</ElRadio>
          <ElRadio :label="true">{{
            t("commerce.maxRedeemByOrderAmountPercents")
          }}</ElRadio>
        </ElRadioGroup>
    ...
    <div class="flex items-center space-x-4">
      <template v-if="item.isPercent">
        <span class="dark:text-[#cfd3dc]">{{
          t("commerce.orderPercentMaxRedeemTip", { percent: item.value })
        }}</span>
      </template>
      <template v-else>
        <span class="dark:text-[#cfd3dc]">{{
          t("commerce.orderMaxRedeemTip", { points: item.value })
        }}</span>
      </template>
    </div>
  </div>
