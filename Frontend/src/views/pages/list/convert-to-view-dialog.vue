...
import { ref } from "vue";
import { getOuterHtml, getViewNames, toView, getViewBody } from "@/api/pages";
...
const onLoad = async () => {
  ...
  if (model.value.isCreate) {
    ...
    viewNames.value = await getViewNames();
  }
};

const loadViewCode = async () => {
  if (model.value.name) {
    model.value.code = "";
    model.value.code = await getViewBody(model.value.name);
  }
};

const onSave = async () => {
  const taskId = await toView(
    props.data.pages,
    model.value.name,
    model.value.isCreate ? model.value.code : "",
      ...
      <el-form-item prop="name">
        ...
        >
          <el-radio-button :label="true">{{
            t("page.createNewView")
          }}</el-radio-button>
          <el-radio-button :label="false">{{
            t("page.convertToExistingView")
          }}</el-radio-button>
        </el-radio-group>
      </el-form-item>
