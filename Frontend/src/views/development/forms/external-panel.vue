...
const onDeletes = async (items: Form[]) => {
  ...
  await formStore.deleteForms(items.map((m) => m.id));
};
</script>

<template>
  <div class="flex items-center space-x-16 pb-12">
    <el-button
      v-hasPermission="{
        feature: 'form',
        action: 'edit',
      ...
      round
      data-cy="create-form"
      @click="$router.push(useRouteSiteId({ name: 'form-edit' }))"
    >
  ...
  >
    ...
    <el-table-column :label="t('common.lastModified')" data-cy="last-modified">
