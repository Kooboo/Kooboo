...
import { getText } from "@/api/module/files";
import { getSetting, updateSetting } from "@/api/module/setting";
...
(async () => {
  var rsp = await getText("root", props.id, "module.config");
  config.value = JSON.parse(rsp);
  const oldSetting = await getSetting(props.id);
  if (oldSetting) {
...
const onSave = async () => {
  ...
  await updateSetting(props.id, JSON.stringify(model.value, null, 2));
  show.value = false;
};
</script>
