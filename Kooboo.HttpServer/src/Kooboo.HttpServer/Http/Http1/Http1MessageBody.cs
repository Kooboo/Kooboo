...
namespace Kooboo.HttpServer.Http
{
    public abstract class Http1MessageBody : MessageBody
    {
            ...
            Http1Connection context)
        {
            ...
            if (!object.ReferenceEquals(context.HttpRequestFeature.Method, HttpMethods.Get))
            {
                // If we got here, request contains no Content-Length or Transfer-Encoding header.
                // Reject with 411 Length Required.
                if (HttpMethods.IsPost(context.HttpRequestFeature.Method) || HttpMethods.IsPut(context.HttpRequestFeature.Method))
                {
                    var requestRejectionReason = httpVersion == HttpVersion.Http11 ? RequestRejectionReason.LengthRequired : RequestRejectionReason.LengthRequiredHttp10;
                    context.ThrowRequestRejected(requestRejectionReason, context.HttpRequestFeature.Method);
                }
            }
