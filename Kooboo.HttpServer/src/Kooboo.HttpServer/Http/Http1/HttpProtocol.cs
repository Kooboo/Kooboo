...
namespace Kooboo.HttpServer.Http
{
    public abstract partial class HttpProtocol : IHttpResponseControl
    {
        ...
        public HttpRequestFeature HttpRequestFeature
        {
            get
            {
                return _context.Features.Request;
            }
            set
            {
                _context.Features.Request = value;
            }
        }
        public HttpResponseFeature HttpResponseFeature
        {
            get
            {
                return _context.Features.Response;
            }
            set
            {
                _context.Features.Response = value;
            }
        }
        ...
        public void InitializeStreams(MessageBody messageBody)
        {
            if (_streams == null)
            {
                _streams = new Streams(AllowSynchronousIO, httpResponseControl: this);
            }
            var pair = _streams.Start(messageBody);
            HttpRequestFeature.Body = pair.Key;
            HttpResponseFeature.Body = pair.Value;
        }
        ...
        public void Reset()
        {
            ...
            HttpRequestFeature = new HttpRequestFeature();
            HttpResponseFeature = new HttpResponseFeature();
            //Method = null;
            //PathBase = null;
            //Path = null;
            //RawTarget = null;
            //QueryString = null;
            _httpVersion = Http.HttpVersion.Unknown;
            HttpResponseFeature.StatusCode= StatusCodes.Status200OK;
            ...
            HttpRequestFeature.Headers = HttpRequestHeaders;
            HttpResponseFeature.Headers = HttpResponseHeaders;

            if (HttpRequestFeature.Scheme == null)
            {
                HttpRequestFeature.Scheme = _context.IsHttps ? "https" : "http";
            }
        ...
        public void OnHeader(Span<byte> name, Span<byte> value)
        {
            ...
            // Kooboo: Modify by suning to remove http header ASCII check 
            var valueString = value.GetKoobooAsciiStringNonNullCharacters();
        ...
        public async Task ProcessRequestsAsync<TContext>(IHttpApplication<TContext> application)
        {
                ...
                while (_keepAlive)
                {
                    ...
                    try
                    {
                        ...
                        else if (!HasResponseStarted)
                        {
                            ...
                            HttpResponseFeature.StatusCode = 0;
                        }
                    }
            ...
            {
                Log.LogError(CoreStrings.RequestProcessingEndError, ex);
            }
            finally
            {
                try
                ...
                {
                    Log.LogError(CoreStrings.ConnectionShutdownError, ex);
                }
                finally
                {
        ...
        private Task FireOnStartingMayAwait(Stack<KeyValuePair<Func<object, Task>, object>> onStarting)
        {
            try
            {
                ...
                for (var i = 0; i < count; i++)
                {
                    ...
                    if (!ReferenceEquals(task, TaskHelpers.CompletedTask()))
                    {
        ...
        public Task FlushAsync(CancellationToken cancellationToken = default(CancellationToken))
        {
            if (!HasResponseStarted)
            {
                ...
                if (!ReferenceEquals(initializeTask, TaskHelpers.CompletedTask()))
                {
        ...
        public Task WriteAsync(ArraySegment<byte> data, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (_canHaveBody)
            {
                if (_autoChunk)
                {
                    if (data.Count == 0)
                    {
                        return !firstWrite ? TaskHelpers.CompletedTask() : FlushAsync(cancellationToken);
                    }
        ...
        protected void VerifyResponseContentLength()
        {
            ...
            if (!HttpMethods.IsHead(HttpRequestFeature.Method) &&
                !responseHeaders.HasTransferEncoding &&
                responseHeaders.ContentLength.HasValue &&
                _responseBytesWritten < responseHeaders.ContentLength.Value)
            {
        ...
        public void ProduceContinue()
        {
            ...
            if (_httpVersion != Http.HttpVersion.Http10 &&
                HttpRequestFeature.Headers.TryGetValue("Expect", out var expect) &&
        ...
        public Task InitializeResponseAsync(int firstWriteByteCount)
        {
            ...
            // If return is Task.CompletedTask no awaiting is required
            if (!ReferenceEquals(startingTask, TaskHelpers.CompletedTask()))
            {
        ...
        private Task WriteSuffix()
        {
            ...
            if (HttpMethods.IsHead(HttpRequestFeature.Method) && _responseBytesWritten > 0)
            {
        ...
        private void CreateResponseHeader(bool appCompleted)
        {
            ...
            _canHaveBody = StatusCanHaveBody(HttpResponseFeature.StatusCode) && HttpRequestFeature.Method != "HEAD";
            ...
            if (_canHaveBody)
            {
                ...
                {
                    if (appCompleted && HttpResponseFeature.StatusCode != StatusCodes.Status101SwitchingProtocols)
                    {
                    ...
                    else
                    {
                        ...
                        if (_httpVersion == Http.HttpVersion.Http11 && HttpResponseFeature.StatusCode != StatusCodes.Status101SwitchingProtocols)
                        {
                            _autoChunk = true;
            ...
            Output.WriteResponseHeaders(HttpResponseFeature.StatusCode,HttpResponseFeature.ReasonPhrase, responseHeaders);
        }
        ...
        {
            var ex = new InvalidOperationException(CoreStrings.FormatHeaderNotAllowedOnResponse("Transfer-Encoding", HttpResponseFeature.StatusCode));
            if (!appCompleted)
            {
        ...
        private void SetErrorResponseHeaders(int statusCode)
        {
            ...
            HttpResponseFeature.StatusCode = statusCode;
            HttpResponseFeature.ReasonPhrase = null;
        ...
        public void HandleNonBodyResponseWrite()
        {
            ...
            if (HttpRequestFeature.Method != "HEAD")
            {
                // Throw Exception for 204, 205, 304 responses.
                throw new InvalidOperationException(CoreStrings.FormatWritingToResponseBodyNotSupported(HttpResponseFeature.StatusCode));
            }
        }
