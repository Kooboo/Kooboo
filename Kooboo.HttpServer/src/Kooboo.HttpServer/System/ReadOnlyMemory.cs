...
namespace System
{
    ...
    public struct ReadOnlyMemory<T>
    {
        ...
        /// </summary>
        public static implicit operator ReadOnlyMemory<T>(T[] array) => (array != null) ? new ReadOnlyMemory<T>(array) : default(ReadOnlyMemory<T>);
        ...
        {
            MemoryHandle memoryHandle = default(MemoryHandle);
            if (pin)
            {
                if (_index < 0)
                {
                ...
                else if (typeof(T) == typeof(char) && _object is string s)
                {
                    ...
                    // Suning: walkaround
                    void* pointer = (void*)Unsafe.Add<byte>(ref Unsafe.AsRef<byte>((void*)handle.AddrOfPinnedObject()), _index);
#else
