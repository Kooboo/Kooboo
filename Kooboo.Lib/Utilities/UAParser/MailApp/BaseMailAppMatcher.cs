...
{
    public abstract class BaseMailAppMatcher : IMailAppMatcher
    {
        public BaseMailAppMatcher()
        {
        }

        public abstract string MailAppName { get; }
        ...
        public bool TryMatch(string userAgentString, out ApplicationInfo application)
        {
            ...
            if (!string.IsNullOrEmpty(find))
            {
                ...
                application = new ApplicationInfo(MailAppName, version);
                return true;
            }
        ...
        public virtual string GetVersion(string identifier, string userAgentString)
        {
            ...
            if (startIndex > 0)
            {
                ...
                if (endIndex > 0)
                {
                    var webBrowserDescription = userAgentString.Substring(startIndex, endIndex);
